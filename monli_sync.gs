...
function doGet(e) {
  try {
    ...
    if (action === 'list') {
      const ss = SpreadsheetApp.getActiveSpreadsheet();
      _ensureTxSheet(ss);
      return _ok({ transactions: _getAllTxObjects(ss) });
    }

    return _ok({ message: 'Monli Sync API aktif' });
  } catch (err) {
    return _err(err.message);
  }
}
...
function _getAllTxObjects(ss) {
  return _getAllTxData(ss).map(_rowToTx).filter(Boolean);
}

function _rowToTx(row) {
  const cat = CATEGORIES.find(c => c.id === row[4]) || {
    id: row[4],
    name: row[5],
    type: row[6] === 'Tabungan' ? 'saving' : 'expense',
  };
  const date = _rowDateToIso(row[1]);
  if (!row[0] || !row[4] || !date) return null;
